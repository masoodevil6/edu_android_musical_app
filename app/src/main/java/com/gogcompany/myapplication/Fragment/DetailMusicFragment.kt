...
class DetailMusicFragment() :Fragment(){
    ...
    override fun onCreateView(
        ...
        layout = inflater.inflate(R.layout.fragment_details_music , container , false);
        cast();
        getParams();
        getResultDb();
        setViews()
    ...
    private fun cast(){
        imageFull =           layout.findViewById(R.id.element_id_fragment_detail_music_image_music);
        imageCard =           layout.findViewById(R.id.element_id_fragment_detail_music_image_in_card_for_image_music);
        btnDownload =         layout.findViewById(R.id.element_id_fragment_detail_music_btn_download_music);
        btnBack =             layout.findViewById(R.id.element_id_fragment_detail_music_btn_back_music);
        btnFav =              layout.findViewById(R.id.element_id_fragment_detail_music_btn_fav_music);
        btnFavImage =         layout.findViewById(R.id.element_id_fragment_detail_music_image_fav_music);
        textMusicName =       layout.findViewById(R.id.element_id_fragment_detail_music_text_music_name);
        textArtistName =      layout.findViewById(R.id.element_id_fragment_detail_music_text_artist_name);
        textDurationTime =    layout.findViewById(R.id.element_id_fragment_detail_music_text_music_time_duration);
        textPlayTime =        layout.findViewById(R.id.element_id_fragment_detail_music_text_music_time_play);
        btnForward =          layout.findViewById(R.id.element_id_fragment_detail_music_btn_forward_music);
        btnReplay =           layout.findViewById(R.id.element_id_fragment_detail_music_btn_replay_music);
        btnPlay =             layout.findViewById(R.id.element_id_fragment_detail_music_btn_play_music);
        seekBar =             layout.findViewById(R.id.element_id_fragment_detail_music_seek_arc);
    ...
    private fun getResultDb(){
        musicData = Base.dbApp!!.musicDao().getInfoMusic(musicId);
        if (musicData!= null){
            artistData = Base.dbApp!!.artistDao().getInfo(musicData!!.artistId);
        }
    }

    private fun setViews(){
        if (musicData != null && artistData != null){
            Picasso.get().load(musicData!!.musicLinkImage).into(imageFull);
            Picasso.get().load(musicData!!.musicLinkImage).into(imageCard);
            textMusicName.text = musicData!!.musicName;
            textArtistName.text = artistData!!.artistName;
            textDurationTime.text = musicData!!.musicTime;
            ...
            if (Base.dbApp!!.favMusicDao().isFavorite(musicId!!) == 1){
            ...
            btnFav.setOnClickListener{
                if (Base.dbApp!!.favMusicDao().isFavorite(musicId!!) != 1){
                    ...
                    favMusic.musicId = musicData!!.musicId;
                    favMusic.artistId = musicData!!.artistId;
                    favMusic.musicIdSort = musicData!!.musicIdSort;
                    favMusic.musicCategory = musicData!!.musicCategory;
                    favMusic.musicLinkMusic = musicData!!.musicLinkMusic;
                    favMusic.musicLinkImage = musicData!!.musicLinkImage;
                    favMusic.musicName = musicData!!.musicName;
                    favMusic.musicTime = musicData!!.musicTime;
    ...
    companion object{
        ...
        var musicData: Musics? = null;
        var artistData: Artists? = null;

        class Player(): AsyncTask<Any , Any , Any>(){
            override fun doInBackground(objects: Array<Any?>): Any? {
                try {
                    mediaPlayer.setDataSource("" + musicData!!.musicLinkMusic)
                    mediaPlayer.prepare();
                }
